...
function renderHabits() {
    if (habits.length === 0) {
        habitsList.innerHTML = '<div class="empty-state"><p>No habits yet. Start building your routine!</p></div>';
        return;
    }
    
    let habitsHTML = '';
    habits.forEach(habit => {
        const lastCompletedHTML = habit.lastCompleted ? '<span class="last-completed">Last: ' + habit.lastCompleted + '</span>' : '';
        
        habitsHTML += '<div class="habit-card ' + (habit.completedToday ? 'completed' : '') + '">' +
...
function updateStats() {
    ...
    completedTodayEl.textContent = habits.filter(h => h.completedToday).length;
    longestStreakEl.textContent = habits.length > 0 ? Math.max(...habits.map(h => h.streak)) : 0;
}
...
function loadHabits() {
    ...
    const habitsCookie = cookies.find(c => c.startsWith('habits='));
